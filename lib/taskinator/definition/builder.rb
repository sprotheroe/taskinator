module Taskinator
  module Definition
    class Builder
      ...
      def on_completed_task(method, options={})
        raise ArgumentError, 'method' if method.nil?
        raise NoMethodError, method unless @executor.respond_to?(method)

        define_on_completed_task(@process, method, @args, options)
        nil
      end
      ...
      def on_failed_task(method, options={})
        raise ArgumentError, 'method' if method.nil?
        raise NoMethodError, method unless @executor.respond_to?(method)

        define_on_failed_task(@process, method, @args, options)
        nil
      end
    ...
    private
      ...
      def define_on_completed_task(process, method, args, options={})
        add_on_completed_task(process) {
          Task.define_step_task(process, method, args, combine_options(options))
        }
      end
      ...
      def define_on_failed_task(process, method, args, options={})
        add_on_failed_task(process) {
          Task.define_step_task(process, method, args, combine_options(options))
        }
      end
      ...
      def add_on_completed_task(process)
        process.on_completed_tasks << task = yield
        task
      end

      def add_on_failed_task(process)
