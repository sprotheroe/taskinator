...
describe Taskinator::Definition::Builder do
  ...
  describe "#on_completed_task" do
    it "creates a task" do
      expect(Taskinator::Task).to receive(:define_step_task).with(process, :task_method, args, builder_options)
      subject.on_completed_task(:task_method)
    end

    it "fails if task method is nil" do
      expect {
        subject.on_completed_task(nil)
      }.to raise_error(ArgumentError)
    end

    it "fails if task method is not defined" do
      expect {
        subject.on_completed_task(:undefined)
      }.to raise_error(NoMethodError)
    end

    it "includes options" do
      expect(Taskinator::Task).to receive(:define_step_task).with(process, :task_method, args, builder_options.merge(options))
      subject.on_completed_task(:task_method, options)
    end
  end
  ...
  describe "#on_failed_task" do
    it "creates a task" do
      expect(Taskinator::Task).to receive(:define_step_task).with(process, :task_method, args, builder_options)
      subject.on_failed_task(:task_method)
    end

    it "fails if task method is nil" do
      expect {
        subject.on_failed_task(nil)
      }.to raise_error(ArgumentError)
    end

    it "fails if task method is not defined" do
      expect {
        subject.on_failed_task(:undefined)
      }.to raise_error(NoMethodError)
    end

    it "includes options" do
      expect(Taskinator::Task).to receive(:define_step_task).with(process, :task_method, args, builder_options.merge(options))
      subject.on_failed_task(:task_method, options)
    end
  end
